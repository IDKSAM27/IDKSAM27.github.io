...
export default function ServiceCatalog() {
  ...
  return <DocumentFrame title="Service catalog" description="Every documented workload, dependency, and platform service in the supplied homelab configuration." section="Homelab" provenance="verified" sections={[{ id: 'catalog', title: 'All services' }, { id: 'reading', title: 'How to read service pages' }, { id: 'references', title: 'References' }]}>
    ...
    <Section id="catalog" title="All services"><div className="eng-index">{Array.from(groups.entries()).map(([category, entries]) => <div className="eng-index-group" key={category}><h3>{category}</h3>{entries.map((service) => <Link href={`/engineering/homelab/services/${service.slug}`} key={service.slug}><span>{service.name}</span><small>{service.summary}</small><b aria-hidden="true">↗</b></Link>)}</div>)}</div></Section>
    ...
    <Section id="references" title="References"><p><Link href="/engineering/homelab/architecture">Architecture overview</Link> · <Link href="/engineering/homelab/domain-map">Domain map</Link> · <Link href="/engineering/homelab/security-model">Security model</Link></p></Section>
  </DocumentFrame>;
}
