...
export default function EngineeringPage() {
  ...
  return <div className="eng-landing">
    ...
    <section className="eng-map" aria-labelledby="documentation-map"><div className="eng-section-heading"><p className="eng-section-number">02</p><h2 id="documentation-map">Documentation map</h2></div><div className="eng-route-index">{routes.map(([title, href, copy], index) => <Link href={href} key={href}><span>{String(index + 1).padStart(2, '0')}</span><div><h3>{title}</h3><p>{copy}</p></div><ArrowUpRight aria-hidden="true" /></Link>)}</div></section>
